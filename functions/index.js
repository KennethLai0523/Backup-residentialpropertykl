...
exports.importListingFromText = onRequest(
  {
    secrets: [openaiKey],
    timeoutSeconds: 120,
    memory: "1GiB",
  ...
  async (req, res) => {
    try {
      const { text, url, imageUrls = [] } = req.body || {};

      if (!text) {
        return res.status(400).json({ error: "Missing text" });
      }

      const finalText = text.slice(0, 12000);

      console.log("BROWSER TEXT LENGTH:", finalText.length);
      console.log("BROWSER TEXT PREVIEW:", finalText.slice(0, 500));
